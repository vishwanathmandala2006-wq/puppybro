...
const server = app.listen(PORT, async () => {
    ...
    // Initialize PostgreSQL schema if available (non-blocking)
    if (db && db.initializeSchema && typeof db.initializeSchema === 'function') {
        // Run schema initialization in the background, don't wait for it
        db.initializeSchema().catch(err => {
            console.warn(`⚠️  Background: Database schema initialization issue: ${err.message}`);
        });
    }
});
